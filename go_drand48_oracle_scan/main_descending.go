...
func main() {
	...
	if *type3Hex == "" || *type4Hex == "" || *dumpOnceHex == "" {
	...
	go func() {
		...
		for {
			...
			case <-ticker.C:
				...
				pc := atomic.LoadUint64(&processedCandidates)
				...
				fmt.Printf("processed_seeds=%s/%s candidates=%s stage1=%s strict=%d rate=%.1f seeds/s eta=%s\n",
					comma(ps), comma(totalSeeds), comma(pc), comma(s1), strictNow, rate, fmtSecs(eta))
			}
		}
	}()
