...
var optimizedKnownSeeds = []uint32{
	3824015219, 2851492325, 2780395207, 2196460210, 1964177823,
	3765470317, 600467025, 3409364283, 1471041271, 3030724834,
	1634202772, 2978206399, 3633125972, 713806982, 284130796,
	523886394, 809496195, 2098923695, 995284052, 2010031080,
	2571777921, 4101089474, 3551943739, 837728196, 3754945813,
	2675176738, 2425737179, 4245513479, 1463428915, 2838917246,
}
...
func main() {
	...
	optimized := flag.Bool("optimized", false, "scan known recovered seeds first, then the remaining range in the normal order")
	stopOnFirstStrict := flag.Bool("stop-on-first-strict", true, "stop issuing new seeds after first strict match")
	printStage1 := flag.Bool("print-stage1", false, "print each stage-1 pass immediately as it is found")
	...
	go func() {
		...
		if len(seedList) > 0 {
			...
			optimizedSeen := make(map[uint32]struct{})
			if *optimized {
				for _, s := range optimizedKnownSeeds {
					if s < startSeed32 || s > endSeed32 {
						continue
					}
					if _, ok := optimizedSeen[s]; ok {
						continue
					}
					optimizedSeen[s] = struct{}{}
					if !sendSeed(s) {
						break
					}
				}
	...
	rep.Inputs.Optimized = *optimized
